...
from hack_rest.route.aadhar_service.models.aadhar_services import AADHAR_VALIDATE_MODEL, AADHAR_RESPONSE_MODEL
...
class AadharValidate(Resource):
    ...
    def post(self):
        ...
        if not aadhar:
            return {"valid": False, "message": "Aadhar number not found"}, HTTPStatus.NOT_FOUND
        ...
        if (
            ...
            return {"valid": False, "message": "Aadhar details do not match"}, HTTPStatus.UNAUTHORIZED
...
class AadharRegister(Resource):
    ...
    def post(self):
        ...
        existing = db.session.query(AadharService).filter(
            AadharService.aadhar_number == data["aadhar_number"]
        ).first()
        if existing:
            return {"valid": False, "message": "Aadhar number already registered"}, HTTPStatus.CONFLICT
        ...
        try:
            ...
            db.session.commit()
            return {"valid": True, "message": "Aadhar registered successfully"}, HTTPStatus.CREATED
